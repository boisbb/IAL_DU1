...
void untilLeftPar ( tStack* s, char* postExpr, unsigned* postLen ) {
  char *top_char = malloc(sizeof(char));
  if(stackEmpty(s) == 0)
    stackTop(s, top_char);
...
void doOperation ( tStack* s, char c, char* postExpr, unsigned* postLen ) {
  ...
  if (c >= 65 || (c >= 48 && c <= 57)) {
    ...
    switch (c){
      ...
      case '-':
        while (1) {
          if(stackEmpty(s) || (*top_char != '*' && *top_char != '/' && *top_char != '+' && *top_char != '-' && *top_char != ')' && *top_char != '(') || *top_char == '('){
            stackPush(s, c);
            break;
          }
          else if (*top_char == '*' || *top_char == '/' || *top_char == '+' || *top_char == '-'){
            postExpr[*postLen] = *top_char;
            postLen[0]++;
            stackPop(s);
          }
          if (stackEmpty(s) == 0) stackTop(s, top_char);
        }
        break;
      case '/':
      case '*':
        while (1){
          if((stackEmpty(s)) || *top_char == '(' || *top_char == '+' || *top_char == '-'){
            stackPush(s, c);
            break;
          }
          else if(*top_char == '*' || *top_char == '/'){
            postExpr[*postLen] = *top_char;
            postLen[0]++;
            stackPop(s);
          }
...
char* infix2postfix (const char* infExpr) {
  ...
  unsigned* postLen = malloc(sizeof(unsigned) * 1);
  *postLen = 0;
  stackInit(s);

  for (int i = 0; '\0' != infExpr[i]; i++){
